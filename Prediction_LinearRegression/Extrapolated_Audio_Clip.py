# You've probably seen a movie or TV show where the super spy agents use some unbelievable technique to enhance a
#  pixelated image, or to recover some lossy data. In this lab, you're going to do something similar—this time your
# objective is to use linear regression to recover or 'fill out' a completely deleted portion of an audio file!
# To complete this lab, you will be using The FSDD, Free-Spoken-Digits-Dataset, an audio dataset put together by
# Zohar Jackson once he noticed there weren't very many cleaned up audio (no dead-space, roughly same length,
# same bitrate, same samples-per-second rate, etc) audio libraries ready for machine learning.
...
#
# .. your code here ..
filedir='Datasets/recordings/'

for i in list(range(50)):
    filename=filedir+'0_jackson_'+str(i)+'.wav'
    sample_rate, data=wavfile.read(filename)
    zero.append(data)

print (sample_rate, zero)
...
wavfile.write('Original_Test_Audio_Clip.wav', sample_rate, test)
...
#
# .. your code here ..
portion=int(np.ceil(Provided_Portion*n_audio_samples))
print (portion)
X_test=test[0:portion]
print (X_test.shape)
...
wavfile.write('Extrapolated_Audio_Clip.wav', sample_rate, completed_clip[0])
