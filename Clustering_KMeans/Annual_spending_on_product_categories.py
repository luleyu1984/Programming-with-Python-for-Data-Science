#Feature scaling was first discussed within one of the PCA lab assignments, but this lab will really familiarize
# you with it. You will be making use of the Wholesale Customer's dataset, hosted by UCI's Machine Learning.
# Unsupervised clustering scans your features and then groups your samples based off of them. Therefore you
# should have a solid understanding of what each of your features are, which one's you should remove, and how to
# scale them in order for the 'blind' clustering to preform correctly and do what you want it to do.
#The first thing that needs to be answered is, what is it you'd like to accomplish by clustering this dataset?
# There are a couple of potential questions you could ask given the data, and the one you choose will drive how
# you manipulate your dataset. Are you interested in which products people buy together, so that you can place
# them near one another in your store, or recommend the pair product when shopping online? Perhaps you're more
# interested in which products people are spending the most money on? Or maybe your interest just lies in identifying
# what individual products people are buying. All of this must be considered.
# For the purposes of this lab, you'll assume you're interested in overall customer behavior rather than channel
# or region specific behavior, so you'll drop those two fields from the dataset. If you were a large wholesaler
# with branches all over the nation, you'd want to keep those fields in so that you can duplicate the process
# outlined in the assignment to particular areas and vertical markets.
...
nans=df.isnull().values.any() #df.isnull().values.sum() the total number of nans
...
ax.set_title('annual spending (m.u.) on diverse product categories')


# Add the cluster label back into the dataframe and display it:
df['label'] = pd.Series(labels, index=df.index)

plt.savefig('Annual_spending_on_product_categories.png', bbox_inches='tight', dpi=300)
plt.show()
